import 'package:flutter/material.dart';

abstract class CashbackColors {
  static final accentColor = Color(0xff0080F6);
  static final accentDisabledColor = Color(0x800080F6);
  static final backgroundColor = Color(0xffFFFFFF);
  static final photoBackgroundColor = Color(0xff000000);
  static final disabledColor = Color(0xff8D95A7);
  static final shadowColor = Color(0x1a000000);
  static final mainTextColor = Color(0xff333333);
  static final contrastTextColor = Color(0xffffffff);
  static final photoCropBorderColor = Color(0xffffffff);
  static final photoAddPlusBackgroundColor = Color(0xffF7F8FA);
}
